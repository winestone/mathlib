...
protected lemma to_map_ne_zero_of_mem_non_zero_divisors [nontrivial R]
  (hM : M ≤ non_zero_divisors R) (x : non_zero_divisors R) : (algebra_map R S) x ≠ 0 :=
map_ne_zero_of_mem_non_zero_divisors (is_localization.injective S hM)
...
protected lemma to_map_ne_zero_of_mem_non_zero_divisors [nontrivial R]
  (x : non_zero_divisors R) : algebra_map R K x ≠ 0 :=
is_localization.to_map_ne_zero_of_mem_non_zero_divisors _ (le_refl _) x
...
lemma mk'_eq_div {r : A} {s : non_zero_divisors A} :
  mk' K r s = algebra_map A K r / algebra_map A K s :=
mk'_eq_iff_eq_mul.2 $ (div_mul_cancel (algebra_map A K r)
    (is_fraction_ring.to_map_ne_zero_of_mem_non_zero_divisors s)).symm
...
lift $ is_unit_map_of_injective hg
...
noncomputable def map [algebra B L] [is_fraction_ring B L] {j : A →+* B} (hj : injective j) :
  K →+* L :=
map K L (λ y, mem_non_zero_divisors_iff_ne_zero.2 $ map_ne_zero_of_mem_non_zero_divisors hj)
...
begin
  ...
  refine mul_left_cancel'
    (is_fraction_ring.to_map_ne_zero_of_mem_non_zero_divisors ⟨c' * b', b_nonzero⟩) _,
...
begin
  ...
  have d_ne_zero : algebra_map A K (denom A x) ≠ 0 :=
    is_fraction_ring.to_map_ne_zero_of_mem_non_zero_divisors (denom A x),
  use ↑d⁻¹ * num A x,
