...
@[simp] lemma mem_canonical_equiv_apply {I : fractional_ideal S P} {x : P'} :
  x ∈ canonical_equiv S P P' I ↔
    ∃ y ∈ I, is_localization.map P P'
      (λ y : S, show (ring_equiv.refl R : R →+* R) y ∈ S, from y.2) y = x :=
begin
...
begin
  ...
  { rw [ne.def, ← @is_fraction_ring.to_map_eq_zero_iff R _ K, hx, algebra.smul_def],
    exact mul_ne_zero (is_fraction_ring.to_map_ne_zero_of_mem_non_zero_divisors _) y_ne_zero },
  { rw hx,
...
lemma canonical_equiv_span_singleton {P'} [comm_ring P'] [algebra R P'] [is_localization S P']
  ...
  canonical_equiv S P P' (span_singleton S x) =
    span_singleton S (is_localization.map P P' (λ (y : S), show ring_hom.id R y ∈ S, from y.2) x) :=
begin
  apply ext_iff.mp,
  intro y,
  split; intro h,
...
begin
  ...
  have h_gx : algebra_map R₁ K x ≠ 0,
    from mt ((algebra_map R₁ K).injective_iff.mp is_fraction_ring.injective x) hx,
