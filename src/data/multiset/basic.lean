...
lemma nsmul_cons {s : multiset α} (n : ℕ) (a : α) : n • (a ::ₘ s) = n • (a ::ₘ 0) + n • s :=
begin
  induction n with n ih,
...
begin
  rw eq_repeat,
  split,
  { rw [add_monoid_hom.map_nsmul, card_repeat, nat.nsmul_eq_mul] },
...
theorem filter_cons {a : α} (s : multiset α) :
  filter p (a ::ₘ s) = (if p a then a ::ₘ 0 else 0) + filter p s :=
begin
  split_ifs with h,
  { rw [filter_cons_of_pos _ h, cons_add, zero_add] },
