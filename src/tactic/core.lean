...
/-- A modified version of `orelse` that, upon failure of the first tactic, keeps its output state
and propagates it to the second tactic. -/
meta def orelse_propagating_state
  (t₁ t₂ : interaction_monad σ α) : interaction_monad σ α :=
...
To make sure that instances found by a failing `apply` are not forgotten, we use instead of `<|>`
a modified version keeping the state even after failure of the first tactic.
-/
meta def apply_list_expr (opt : apply_cfg) : list (tactic expr) → tactic unit
| []     := fail "no matching rule"
| (h::t) :=
@interaction_monad.orelse_propagating_state tactic_state _
