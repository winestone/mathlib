...
noncomputable
instance quotient.semi_normed_group (S : add_subgroup M) : semi_normed_group (quotient S) :=
{ dist               := λ x y, ∥x - y∥,
...
(quotient.semi_normed_group S).to_uniform_space.to_topological_space :=
rfl
...
example (S : add_subgroup M) [is_closed (S : set M)] :
  quotient.semi_normed_group S = normed_group.to_semi_normed_group := rfl
...
lemma normed_group.mk.ker (S : add_subgroup M) : S.normed_mk.ker = S :=
quotient_add_group.ker_mk  _

/-- The operator norm of the projection is at most `1`. -/
lemma norm_quotient_mk_le (S : add_subgroup M) : ∥S.normed_mk∥ ≤ 1 :=
normed_group_hom.op_norm_le_bound _ zero_le_one (λ m, by simp [quotient_norm_mk_le])

/-- The operator norm of the projection is `1` if the subspace is not dense. -/
lemma norm_quotient_mk (S : add_subgroup M) (h : (S.topological_closure : set M) ≠ univ) :
  ∥S.normed_mk∥ = 1 :=
begin
...
⟨S.surjective_normed_mk, λ m, by simpa [normed_group.mk.ker S] using quotient_norm_mk_eq _ m⟩
