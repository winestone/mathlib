...
lemma pi_univ_Ioo_subset_Icc (x y : fin n → ℝ) :
  pi univ (λ i, Ioo (x i) (y i)) ⊆ Icc x y :=
λ z hz, ⟨λ i, (hz i trivial).1.le, λ i, (hz i trivial).2.le⟩

lemma Icc_mem_nhds_of_mem_pi_univ_Ioo {x y z : fin n → ℝ}
  (h : z ∈ pi univ (λ i, Ioo (x i) (y i))) :
  Icc x y ∈ 𝓝 z :=
pi_Icc_mem_nhds' (λ i, (h i trivial).1) (λ i, (h i trivial).2)

lemma unique_diff_within_at_pi_Icc_of_mem_pi_univ_Ioo {x y z : fin n → ℝ}
  (h : z ∈ pi univ (λ i, Ioo (x i) (y i))) :
  unique_diff_within_at ℝ (Icc x y) z :=
unique_diff_within_at_of_mem_nhds (Icc_mem_nhds_of_mem_pi_univ_Ioo h)
...
lemma affine_map.box_boundary_integral (hxy : x ≤ y)
  (f : (fin (n + 1) → ℝ) →ᵃ[ℝ] (fin (n + 1) → E)) (hf : continuous f) :
...
begin
  refine box_additive_on_sum_faces_fin (Icc x y)
    (λ (i : fin (n + 1)) c (l r : fin n → ℝ), ∫ z in Icc l r, f (i.insert_nth c z) i)
    (λ i c, box_additive_on_set_integral_Icc _),
...
begin
  ...
  have : box_additive_on (λ l u, (∫ z in Icc l u, divf z) - box_boundary_integral f l u) (Icc x y),
    from (box_additive_on_set_integral_Icc (hf'c.integrable_on_compact compact_pi_Icc)).sub
