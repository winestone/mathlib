...
lemma finset.generate_eq (s : finset E) (x : E) :
  (generate (s : set E) : set E) =
   {x : E | ∃ (w : E → ℝ) (hw₀ : ∀ y ∈ s, 0 ≤ w y)
             (hw₁ : ∑ y in s, w y = 1), s.center_mass w id = x} :=
begin
...
noncomputable def make_integer_point (x : fin n → ℝ) : fin n → ℤ := λ i, floor (x i)
lemma integer_point_eq {x : fin n → ℝ} (hx : is_integer_point x) (i) :
  (make_integer_point x i : ℝ) = x i :=
begin
  simp [make_integer_point],
  obtain ⟨z, hz⟩ := hx i,
  rw hz,
  simp
...
begin
  ...
  { apply finite.preimage _ this,
    ...
    specialize q i,
    simpa using q },
  let s' : finset (fin n → ℤ) := (s.image w).image (make_integer_point),
