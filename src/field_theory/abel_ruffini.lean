...
begin
  ...
  have f : F⟮α, β⟯ →ₐ[F] (p * q).splitting_field := sorry,
  have h₈ : ((⟨γ, hγ⟩ : F⟮α, β⟯.to_subalgebra) : SBR F E) = γ := rfl,
  have h₉ : aeval γ r = 0 := minimal_polynomial.aeval (is_integral γ),
  have h₇ : aeval (⟨γ, hγ⟩ : F⟮α, β⟯) r = 0,
  { rw ← h₈ at h₉,
    rw subalgebra.aeval_coe at h₉,
    squeeze_simp * at *,
    -- rw @subalgebra.aeval_coe F (SBR F E) _ _ _ F⟮α, β⟯ ⟨γ, hγ⟩ r at h₉,
    sorry,
  },
